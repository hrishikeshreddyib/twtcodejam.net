    ...
    def get(self, request: WSGIRequest, teamid):
        ...
        if len(user_teams) == 0:
            messages.add_message(
                request, messages.WARNING, "You have not participated in the code jam"
            )
            client.send_webhook(
                "Teams",
        ...
        if len(Submission.objects.filter(challenge=challenge, team=user_teams[0])) == 0:
            ...
            client.send_webhook(
                ...
                fields=[
                    {
                        "name": "Error",
                        "value": "They have not Submitted a project in the code jam",
                    }
                ],
            )
            return redirect("/")
        if len(user_voted_teams) >= 3:
            messages.add_message(
                request, messages.WARNING, "You have exceeded your amount of votes."
            )
            client.send_webhook(
                "Teams",
                f"<@{discord_user.uid}> tried voting **{team.name}** (ID:{team.ID})",
        ...
        if team in user_teams:
            messages.add_message(
                request, messages.WARNING, "You Cannot vote for yourself."
            )
            client.send_webhook(
                "Teams",
                f"<@{discord_user.uid}> tried voting **{team.name}** (ID:{team.ID})",
        ...
        if team in user_voted_teams:
            messages.add_message(
                request, messages.WARNING, "You Cannot vote this team again."
            )
            client.send_webhook(
                "Teams",
                f"<@{discord_user.uid}> tried voting **{team.name}** (ID:{team.ID})",
        ...
        if not challenge.voting_status:
            messages.add_message(
                request, messages.WARNING, "Challenge Voting is not open."
            )
            client.send_webhook(
                "Teams",
                f"<@{discord_user.uid}> tried voting **{team.name}** (ID:{team.ID})",
        ...
        team.save()
        messages.add_message(
            request, messages.WARNING, f"You have successfully voted {team.name}"
        )
        client.send_webhook(
            "Teams", f"<@{discord_user.uid}> Voted **{team.name}** (ID:{team.ID})"
        )
