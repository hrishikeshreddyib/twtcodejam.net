...
class Submission_View(View):
        ...
        if form.is_valid():
            ...
            if len(Submission.objects.filter(challenge=challenge, team=team)) != 0:
                messages.add_message(
                    request, messages.WARNING, "You have already submitted."
                )
                client.send_webhook(
                    "Submissions",
    ...
    def get(self, request: WSGIRequest) -> HttpResponse:
        if not request.user.is_authenticated:
            messages.add_message(request, messages.INFO, "Sign In")
            return redirect("/")
        ...
        if challenge.submissions_status == False:
            messages.add_message(
                request, messages.WARNING, "Submissions Closed Right Now"
            )
            return redirect("home:home")
        return render(
            request=request, template_name="timathon/submit.html", context=context
        )
