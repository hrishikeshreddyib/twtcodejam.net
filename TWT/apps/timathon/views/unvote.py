...
class UnVote(View):
    ...
    def get(self, request: WSGIRequest, teamid):
        ...
        if len(user_voted_teams) == 0:
            messages.add_message(request, messages.WARNING, "You have not voted yet.")
            client.send_webhook(
                "Teams",
        ...
        if team in user_teams:
            messages.add_message(
                request, messages.WARNING, "You Cannot unvote for yourself."
            )
            client.send_webhook(
                "Teams",
                f"<@{discord_user.uid}> tried unvoting **{team.name}** (ID:{team.ID})",
        ...
        if team not in user_voted_teams:
            messages.add_message(
                request, messages.WARNING, "You have not voted this team."
            )
            client.send_webhook(
                "Teams",
                f"<@{discord_user.uid}> tried unvoting **{team.name}** (ID:{team.ID})",
        ...
        team.save()
        messages.add_message(
            request, messages.WARNING, f"You have successfully unvoted {team.name}"
        )
        client.send_webhook(
            "Teams", f"<@{discord_user.uid}> Unvoted **{team.name}** (ID:{team.ID})"
        )
