...
class EndView(View):
    ...
    def get(self, request: WSGIRequest, challenge_id: int):
        ...
        if context["is_admin"] or context["is_challenge_host"]:
            ...
            # try:
            winners = Team.objects.filter(challenge=challenge, submitted=True).exclude(
                winner=0
            )  # [Team.objects.get(challenge=challenge,winner=i) for i in range(1,4)]
            # except Team.DoesNotExist:
            if len(winners) == 0:
                messages.add_message(
                    request, messages.WARNING, "Declare the winning team first"
                )
            ...
            challenge.save()
            messages.add_message(request, messages.INFO, "Challenge has been closed!")
            client.send_webhook(
                "Code Jam",
