    ...
    def get(self, request: WSGIRequest, challenge_id: int):
        ...
        if context["is_admin"] or context["is_challenge_host"]:
            ...
            messages.add_message(
                request, messages.INFO, "Team Creation Has been stopped"
            )
            client.send_webhook(
                "Timathon",
                f"Team creation process has been stopped",
                fields=[
                    {
                        "name": "Link",
            ...
            client.send_webhook(
                "Code Jam",
                f"<@{context['discord_user'].uid}> has stopped the team creation process",
                fields=[
                    {
                        "name": "Link",
                        "value": f"[Visit]({request.build_absolute_uri('/timathon/')})",
