        ...
        if context["is_admin"] or context["is_challenge_host"]:
            ...
            if challenge.team_creation_status or challenge.voting_status:
                messages.add_message(
                    request, messages.WARNING, "Stop Team creation or voting first"
                )
                return redirect("timathon:Home")
            challenge.submissions_status = True
            challenge.save()
            messages.add_message(
                request, messages.INFO, "Submissions have been started"
            )
            client.send_webhook(
                "Timathon",
                f"Submissions have started",
                fields=[
                    {
                        "name": "Link",
            ...
            client.send_webhook(
                "Code Jam",
                f"<@{context['discord_user'].uid}> has started the submissions",
                fields=[
                    {
                        "name": "Link",
                        "value": f"[Visit]({request.build_absolute_uri('/timathon/')})",
